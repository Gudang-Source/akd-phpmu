<?php 
    if (isset($_POST[simpan])){
            if ($_POST[status]=='Update'){
              mysqli_query(dblink(), "UPDATE rb_nilai_pengetahuan SET kd='$_POST[a]', nilai1='$_POST[b]', nilai2='$_POST[c]', nilai3='$_POST[d]', nilai4='$_POST[e]', nilai5='$_POST[f]', deskripsi='$_POST[g]' where id_nilai_pengetahuan='$_POST[id]'");
            }else{
              mysqli_query(dblink(), "INSERT INTO rb_nilai_pengetahuan VALUES('','$_GET[jdwl]','$_POST[nisn]','$_POST[a]','$_POST[b]','$_POST[c]','$_POST[d]','$_POST[e]','$_POST[f]','$_POST[g]','$_SESSION[id]','".date('Y-m-d H:i:s')."')");
            }
    ...
    if (isset($_GET[delete])){
        mysqli_query(dblink(), "DELETE FROM rb_nilai_pengetahuan where id_nilai_pengetahuan='$_GET[delete]'");
          ...
          <div class='panel-body'>
              <table class='table table-bordered table-striped'>
                              ...
                              while($r=mysqli_fetch_array($tampil)){
                                  if (isset($_GET[edit])){
                                      $e = mysqli_fetch_array(mysqli_query(dblink(), "SELECT * FROM rb_nilai_pengetahuan where id_nilai_pengetahuan='$_GET[edit]'"));
                                      $name = 'Update';
                                  }else{
                                      $name = 'Simpan';
                                  }
                                  if ($_GET[nisn]==$r[nisn]){
                                    ...
                                    $pe = mysqli_query(dblink(), "SELECT * FROM rb_nilai_pengetahuan where kodejdwl='$_GET[jdwl]' AND nisn='$r[nisn]'");
                                    ...
                                    if ($cekpredikat >= 1){
                                      ...
                                      $maxn = mysqli_fetch_array(mysqli_query(dblink(), "SELECT ((nilai1+nilai2+nilai3+nilai4+nilai5)/5) as rata_rata, deskripsi FROM rb_nilai_pengetahuan where kodejdwl='$_GET[jdwl]' AND nisn='$r[nisn]' ORDER BY rata_rata DESC LIMIT 1"));
                                      $cekpredikat1 = mysql_num_rows(mysqli_query(dblink(), "SELECT * FROM rb_predikat where kode_kelas='$_GET[id]'"));
                                      if ($cekpredikat1 >= 1){
                                        $grade2 = mysqli_fetch_array(mysqli_query(dblink(), "SELECT * FROM `rb_predikat` where (".number_format($maxn[rata_rata])." >=nilai_a) AND (".number_format($maxn[rata_rata])." <= nilai_b) AND kode_kelas='$_GET[id]'"));
                                      }else{
                                        $grade2 = mysqli_fetch_array(mysqli_query(dblink(), "SELECT * FROM `rb_predikat` where (".number_format($maxn[rata_rata])." >=nilai_a) AND (".number_format($maxn[rata_rata])." <= nilai_b) AND kode_kelas='0'"));
                                      }
                                      
                                      $rapn = mysqli_fetch_array(mysqli_query(dblink(), "SELECT sum((nilai1+nilai2+nilai3+nilai4+nilai5)/5)/count(nisn) as raport FROM rb_nilai_pengetahuan where kodejdwl='$_GET[jdwl]' AND nisn='$r[nisn]'"));

<?php 
  echo "<div class='col-xs-12'>  
      ...
      echo "<tr>
        ...
        $n = mysqli_fetch_array(mysqli_query(dblink(), "SELECT * FROM rb_nilai_uts where kodejdwl='$m[kodejdwl]' AND nisn='$iden[nisn]'"));
        $cekpredikat = mysql_num_rows(mysqli_query(dblink(), "SELECT * FROM rb_predikat where kode_kelas='$_SESSION[kode_kelas]'"));
        if ($cekpredikat >= 1){
          $grade1 = mysqli_fetch_array(mysqli_query(dblink(), "SELECT * FROM `rb_predikat` where ($n[angka_pengetahuan] >=nilai_a) AND ($n[angka_pengetahuan] <= nilai_b) AND kode_kelas='$_SESSION[kode_kelas]'"));
          $grade2 = mysqli_fetch_array(mysqli_query(dblink(), "SELECT * FROM `rb_predikat` where ($n[angka_keterampilan] >=nilai_a) AND ($n[angka_keterampilan] <= nilai_b) AND kode_kelas='$_SESSION[kode_kelas]'"));
        }else{

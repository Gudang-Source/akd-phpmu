...
if (isset($_POST['login'])){
 ...
 $admin = mysqli_query(dblink(), "SELECT * FROM rb_users WHERE username='".anti_injection($_POST['a'])."' AND password='".$pass."'");
 $guru = mysqli_query(dblink(), "SELECT * FROM rb_guru WHERE nip='".anti_injection($_POST['a'])."' AND password='".$passlain."'");
 $siswa = mysqli_query(dblink(), "SELECT * FROM rb_siswa WHERE nisn='".anti_injection($_POST['a'])."' AND password='".$passlain."'");
 ...
 if ($hitungadmin >= 1){
    ...
    mysqli_query(dblink(), "INSERT INTO rb_users_aktivitas VALUES('','".$r['id_user']."','".$ip."','".$user_browser." $version','".$user_os."','".$r['level']."','".date('H:i:s')."','".date('Y-m-d')."')");
    ...
    $_SESSION['level']    = 'guru';
    include "config/user_agent.php";
    mysqli_query(dblink(), "INSERT INTO rb_users_aktivitas VALUES('','".$r['nip']."','".$ip."','".$user_browser." $version','".$user_os."','guru','".date('H:i:s')."','".date('Y-m-d')."')");
    echo "<script>document.location='index.php';</script>";
    ...
    include "config/user_agent.php";
    mysqli_query(dblink(), "INSERT INTO rb_users_aktivitas VALUES('','".$r['nisn']."','".$ip."','".$user_browser." $version','".$user_os."','siswa','".date('H:i:s')."','".date('Y-m-d')."')");
    echo "<script>document.location='index.php';</script>";
 }else{
